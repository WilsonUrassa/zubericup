      ...
      <div id="admin-panel">
        ...
        {!token ? (
          ...
          <div id="admin-content">
            ...
            <div className="admin-subtitle">Zuberi Cup 2026 · Live Match Control</div>
                ...
                return (
                  <div key={m.id} className="admin-match-row">
                    ...
                    <div className="admin-fields">
                      <div className="admin-field"><label>Timu A</label><input type="text" value={m.team_a} onChange={e => updateLocalMatch(m.id, 'team_a', e.target.value)} /></div>
                      <div className="admin-field"><label>Goli A</label><input type="number" value={m.score_a} min={0} onChange={e => updateLocalMatch(m.id, 'score_a', +e.target.value)} /></div>
                      <div className="admin-field"><label>Goli B</label><input type="number" value={m.score_b} min={0} onChange={e => updateLocalMatch(m.id, 'score_b', +e.target.value)} /></div>
                      <div className="admin-field"><label>Timu B</label><input type="text" value={m.team_b} onChange={e => updateLocalMatch(m.id, 'team_b', e.target.value)} /></div>
                      <div className="admin-field"><label>Dakika</label><input type="number" value={m.minute} min={0} max={120} onChange={e => updateLocalMatch(m.id, 'minute', +e.target.value)} /></div>
                      <div className="admin-field">
                        <label>Hali</label>
                        <select value={m.status} onChange={e => updateLocalMatch(m.id, 'status', e.target.value)}>
                          <option value="upcoming">Inakuja</option><option value="live">LIVE</option><option value="ft">Imekwisha</option>
                        </select>
                      </div>
                    </div>
                    ...
                    <div style={{ marginTop: '16px' }}>
                      <div style={{ fontFamily: 'Barlow Condensed', fontSize: '10px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: 'var(--grey)', marginBottom: '8px' }}>ONGEZA GOLI</div>
                      <div className="admin-goal-input">
                        <div className="admin-field">
                          <label>Jina la Mpiga Goli</label>
                          <input type="text" placeholder="J. Mwangi" value={scInput.name} onChange={e => setScorerInputs(prev => ({ ...prev, [m.id]: { ...scInput, name: e.target.value } }))} />
                        </div>
                        <div className="admin-field">
                          <label>Dakika</label>
                          <input type="number" value={scInput.minute} min={1} max={120} onChange={e => setScorerInputs(prev => ({ ...prev, [m.id]: { ...scInput, minute: e.target.value } }))} />
                        </div>
                      </div>
                    ...
                    {(scA.length > 0 || scB.length > 0) && (
                      <div className="goal-log" style={{ marginTop: '12px' }}>
                        <div style={{ fontFamily: 'Barlow Condensed', fontSize: '10px', letterSpacing: '2px', color: 'var(--grey)', marginBottom: '4px' }}>MAGOLI YALIYOORODHESHWA:</div>
            ...
            <div style={{ fontFamily: 'Barlow Condensed', fontSize: '11px', fontWeight: 700, letterSpacing: '3px', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: '14px' }}>ONGEZA MECHI MPYA</div>
            <div className="admin-fields">
              <div className="admin-field"><label>Timu A</label><input type="text" placeholder="Eagle FC" value={newMatch.team_a} onChange={e => setNewMatch(p => ({ ...p, team_a: e.target.value }))} /></div>
              <div className="admin-field"><label>Goli A</label><input type="number" value={newMatch.score_a} min={0} onChange={e => setNewMatch(p => ({ ...p, score_a: +e.target.value }))} /></div>
              <div className="admin-field"><label>Goli B</label><input type="number" value={newMatch.score_b} min={0} onChange={e => setNewMatch(p => ({ ...p, score_b: +e.target.value }))} /></div>
              <div className="admin-field"><label>Timu B</label><input type="text" placeholder="Pasua FC" value={newMatch.team_b} onChange={e => setNewMatch(p => ({ ...p, team_b: e.target.value }))} /></div>
              <div className="admin-field"><label>Dakika</label><input type="number" value={newMatch.minute} min={0} max={120} onChange={e => setNewMatch(p => ({ ...p, minute: +e.target.value }))} /></div>
              <div className="admin-field">
                <label>Hali</label>
            ...
            <div style={{ marginTop: '12px' }}><button className="btn-admin green" onClick={addMatch}>+ Ongeza Mechi</button></div>
