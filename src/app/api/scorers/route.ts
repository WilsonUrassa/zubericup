...
export async function POST(req: NextRequest) {
  ...
  const { data: scorer, error: scorerErr } = await db
    ...
    .select()
    .single();
  if (scorerErr) return NextResponse.json({ error: scorerErr.message }, { status: 500 });

  // Increment score
  const col = team_side === 'A' ? 'score_a' : 'score_b';
  const { data: match } = await db.from('matches').select(col).eq('id', match_id).single();
  await db
    .from('matches')
...
// DELETE /api/scorers/[id] — remove a scorer
export async function DELETE(req: NextRequest) {
  ...
  const db = createServiceClient();
  // Get scorer info before deleting
  const { data: scorer } = await db.from('scorers').select('*').eq('id', id).single();
  if (!scorer) return NextResponse.json({ error: 'Not found' }, { status: 404 });

  await db.from('scorers').delete().eq('id', id);

  const col = scorer.team_side === 'A' ? 'score_a' : 'score_b';
  const { data: match } = await db.from('matches').select(col).eq('id', scorer.match_id).single();
  await db
    .from('matches')
